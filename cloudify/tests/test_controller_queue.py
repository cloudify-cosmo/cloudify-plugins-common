...
class ControllertTests(testtools.TestCase):
    ...
    def test_controller_queue_property(self, cfy_local):
        ...
        instance = cfy_local.ctx.get_node_instances('connected_host')
        self.assertEqual(
            instance.properties['controller_queue'], 'queue')
        instance = cfy_local.ctx.get_node_instances('direct_override')
        self.assertEqual(
            instance.properties['controller_queue'], 'direct_override')
        instance = cfy_local.ctx.get_node_instances('contained_node')
        self.assertEqual(
